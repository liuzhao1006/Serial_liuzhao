...
import com.lz.base.socket.ClientMessage;
import com.lz.base.socket.ClientStatMachine;
import com.lz.base.socket.Constant;
import com.lz.base.socket.NodeStatus;
import com.lz.base.socket.ServerMessage;
		...
		if(this.writePending == false){
			if(buffer.position() < Constant.CLIENT_MESSAGE_MIN_LENGTH){
				...
				if(bodyLen > Constant.CLIENT_MESSAGE_MAX_LENGTH-Constant.CLIENT_MESSAGE_MIN_LENGTH){
					throw new IllegalArgumentException("content length is "+bodyLen+", larger than the max of "+(Constant.CLIENT_MESSAGE_MAX_LENGTH-Constant.CLIENT_MESSAGE_MIN_LENGTH));
				}
				if(bodyLen == 0){
					this.writePending = true;
				}else{
